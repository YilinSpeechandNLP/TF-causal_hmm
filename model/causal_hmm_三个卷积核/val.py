...
def val(args, causal_hmm_model, classifier, val_dataset, batch_size, writer, epoch, n_iter, val_log):
    ...
    # output_dropout = dropout_layer(val_loader)
    rec_loss_x, kl_loss_x, rec_loss_A, kl_loss_A, pre_loss = 0, 0, 0, 0, 0
    ...
    with torch.no_grad():
        ...
        for i_batch, (sample_batched) in enumerate(val_loader):
            ...
            # image_data = torch.tensor(sample_batched[0])
            image_data_1 = torch.tensor(dropout_layer(sample_batched[0]))
            # print(image_data_1.shape)
            image_data = image_data_1.squeeze(1)
            gt_y = torch.tensor(sample_batched[1])
            # dropout_layer = nn.Dropout(p=0.4)

            A_data = torch.tensor(dropout_layer(sample_batched[2]))
            B_data = torch.tensor(sample_batched[3])
        ...
        val_log['val_log'].info \
            ("val acc:{0}, val auc:{1} at epoch {2},Precision: {3}, Recall: {4}, F1 Score: {5}".format \
                 (all_val_acc, all_val_auc, epoch, precision, recall, f1))
