...
from data_helper import RETINA, PITT
...
parser.add_argument('--cls-loss-weight', type=float, default=0.1, metavar='LR', help='classification loss weight (default: 0.1)')
parser.add_argument('--to-grade', type=int, default=1, help='use data up to grade')
...
def main():
    ...
    causal_hmm_model = Causal_HMM(args, z_size=args.z_size, v_size=args.v_size,s_size=args.s_size,
                      A_size=27, B_size=1, batch_size=args.batch_size, layer_count = args.cnn_layer, channels = 3)
    # 注意修改特征维度时需要改A_size和B_size类似的变量 代码质量一般 需要深入不同文件中去找这类变量的定义
    svm_model_path = 'svm_model.pkl'
    classifier = Disease_Classifier(args, in_dim=args.v_size + args.s_size)
    # classifier = Disease_Classifier(svm_model_path)
    ...
    train_label_path = 'data/train_audio_text_normal.csv'
    val_label_path = 'data/val_audio_text_normal.csv'
    test_label_path = 'data/test_audio_text_normal.csv'
    train_image_path = 'image'
    val_image_path = 'image'
    ...
    train_dataset = PITT(args, train_image_path, train_label_path)
    val_dataset = PITT(args, val_image_path, val_label_path)
    test_dataset = PITT(args, test_image_path, test_label_path)
    ...
    for epoch in range(train_epoch):
        ...
        n_iter = train(args, causal_hmm_model, classifier, train_dataset, batch_size, vae_optimizer,classifier_optimizer
        ...
        f1_test_value=test(args, causal_hmm_model, classifier, test_dataset, batch_size, writer, epoch, n_iter, test_log)
